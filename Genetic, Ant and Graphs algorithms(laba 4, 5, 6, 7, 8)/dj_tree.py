...
class DjtreeGUI:
    def __init__(self, master):
        ...
        self.nodes = []
        self.node_labels = {}
        self.node_indexes = {}  # Словарь для хранения индексов вершин
        self.edges = []
    ...
    def create_node(self, event):
        x, y = event.x, event.y
        # Используем индекс узла в списке self.nodes как его числовой идентификатор
    ...
    def color_path(self, path):
        for i in range(len(path) - 1):
            ...
            for edge in self.edges:
                if (edge[1][0] == start_node and edge[2][0] == end_node) or \
                        (edge[1][0] == end_node and edge[2][0] == start_node):
                    self.canvas.itemconfig(edge[0], fill="red")
                    break
