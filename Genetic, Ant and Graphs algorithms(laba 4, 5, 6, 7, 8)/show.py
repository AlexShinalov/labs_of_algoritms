...
class ShowResult:
    def __init__(self, data, max_i, frame_btn, frame_scroll, snapshot_name):
        ...
        self.photo = None
        self.canvas = Canvas(self.frame_btn, width=420, height=345, background="#d1dbe0", highlightthickness=0)
    ...
    def show_img(self) -> None:
        try:
            self.photo = ImageTk.PhotoImage(Image.open("source/{1}{2}.png".format(self.snapshot_name,
...
class ShowGraph:
    ...
    def __init__(self, frame_btn, frame_scroll, frame_input):
        ...
        self.canvas = Canvas(self.frame_btn, width=500, height=310, background="#d1dbe0", highlightthickness=0)
    ...
    def draw_lines(self) -> None:
        ...
        for p1 in self.points:
            ...
            self.canvas.create_oval(p1[0] - self.point_size, p1[1] - self.point_size,
                                    p1[0] + self.point_size, p1[1] + self.point_size, fill='black')
    ...
    def draw_cycle(self, route) -> None:
        ...
        for i, row in enumerate(self.cycle_table.get_children()):
            ...
            self.canvas.delete("black{0}".format(i))
        # Добавляем новые
        for i in range(len(route) - 1):
            ...
            p2 = self.points[to_]
            self.canvas.create_line(p1[0], p1[1], p2[0], p2[1], fill="black", width=2, tags="black{0}".format(i))
            p1_index = self.points.index(p1) + 1
            p2_index = self.points.index(p2) + 1
